...
inline void Kinect::drawBody()
{
	...
	Concurrency::parallel_for(0, BODY_COUNT, [&](const int count) {
		...
		int ShoulderLeftYaw = 0;
		int ShoulderLeftPitch = 0;
		int ShoulderLeftRoll = 0;
		//SpineBase
		float HandLeftX = 0;
		float  HandLeftY = 0;
		float  HandLeftZ = 0;
		...
		int ShoulderRightYaw = 0;
		int ShoulderRightPitch = 0;
		int ShoulderRightRoll = 0;
		//SpineBase
		float HandRightX = 0;
		float  HandRightY = 0;
		float  HandRightZ = 0;
		...
		for (int i = 0; i < 25; i++) {
			...
			if ((0 <= x) && (x < 1920) && (0 <= y) && (y < 1080)) {
				...
				if (i == 0) {
					...
					ShoulderLeftYaw = Yaw[i];
					ShoulderLeftPitch = Pitch[i];
					ShoulderLeftRoll = Roll[i];
				}
				else if (type == 11) {
					// HandLeft
					...
					ShoulderRightYaw = Yaw[i];
					ShoulderRightPitch = Pitch[i];
					ShoulderRightRoll = Roll[i];
				}
				else if (type == 7) {
					//HandRight
					...
					HandRightYaw = Yaw[i];
					HandRightYaw = Pitch[i];
					HandRightRoll = Roll[i];
				}
				else if (type == 20) {
					//SpineShoulder
